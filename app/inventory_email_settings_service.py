...
Persisted as two named rows in the `inventory_parameters` table (see
...
cloud-sync wiring (parameter sync project, Phase 2/3). This is a
deliberate exclusion, not an oversight: Path Validator's own
AppSettings.gmail_app_password is likewise NEVER pushed to the cloud by
app/email_settings_service.py (confirmed: it has no push_config/pull_config
call anywhere) -- an app password is exactly the kind of credential that
should never leave the machine it was typed into via a shared cloud
config table. Reading/writing InventoryParameter directly here, instead
of going through inventory_parameters_service.py's synced get/set
helpers, keeps these two keys out of that sync path entirely, matching
Path Validator's own security posture for the same kind of secret.
"""
...
_SETTINGS_KEYS = (INVENTORY_SENDER_EMAIL, INVENTORY_SENDER_APP_PASSWORD)
...
def get_settings() -> dict:
    """Return {'sender_email', 'app_password'} -- the Inventory Automated
    Email System's own saved credentials. Values are empty if nothing has
    been saved yet. Mirrors app.email_settings_service.get_settings()'s
    exact shape, minus `master_email` (Inventory has no
    hierarchy-resolution-fallback "master consolidated report" concept --
    every configured recipient IS the full recipient list, there's no
    separate rollup address)."""
    ...
    settings = {"sender_email": sender_email, "app_password": app_password}
    logger.info(f"Inventory email settings loaded: sender={settings['sender_email'] or '(none)'}")
    return settings


def save_settings(sender_email: str, app_password: str) -> None:
    """Upsert both settings. Mirrors
    ...
    logger.info(f"Inventory email settings saved: sender={sender_email or '(none)'}")
