"""The Payment Analytics module: branded sidebar navigation plus page
content, mirroring ui/inventory_module.py's shell shape. UI scaffold only
-- see the individual page modules for what's still placeholder.
"""
...
from ui.payment_dashboard_page import PaymentDashboardPage
from ui.payment_parameters_page import PaymentParametersPage
from ui.payment_upload_page import PaymentUploadPage
...
class PaymentAnalyticsModule(ctk.CTkFrame):
    """The Payment Analytics module: a branded left sidebar and a page
    ...
    def _build_sidebar(self, parent) -> None:
        ...
        ctk.CTkLabel(
            title_box, text="Payment Analytics", font=Font.SMALL, text_color=Color.TEXT_SECONDARY, anchor="w"
        ).pack(anchor="w")
    ...
    def _build_pages(self, parent) -> None:
        ...
        self.pages["Dashboard"] = PaymentDashboardPage(container, on_view_customer=self._on_view_customer)
        self.pages["Historical Upload"] = PaymentUploadPage(
            container, on_continue=lambda: self.show_page("Dashboard"),
            on_view_dashboard=lambda: self.show_page("Dashboard"),
        )
        self.pages["Customer Analytics"] = PaymentCustomerAnalyticsPage(container)
        self.pages["Collections Action Center"] = PaymentCollectionsPage(container)
        self.pages["Parameters"] = PaymentParametersPage(container)
    ...
    def _on_view_customer(self, party_name: str) -> None:
        """Critical Customers row click (Dashboard) -> Customer Analytics,
        pre-filtered to that customer."""
        self.show_page("Customer Analytics")
        self.pages["Customer Analytics"].focus_customer(party_name)
